...
import {
  useFacultyData,
  useFacultySection,
} from "../../../context/FacultyDataContext";
...
export default function ConferenceSessionChairsPage() {
  ...
  const { data: conference_session_data } =
    useFacultySection("conference_session_chairs");
  ...
  const handleSave = async (chair) => {
    ...
    if (res.ok) {
      ...
      let updatedChairs;
      if (isEdit) {
        updatedChairs = chairs.map((c) => (c.id === chair.id ? chair : c));
      } else {
        const newChair = { ...chair, id: result.id || Date.now().toString() };
        updatedChairs = [...chairs, newChair];
      }
            ...
            <TableBody>
              {chairs.length ? (
                chairs
                  .sort(
                    (a, b) =>
                      new Date(a.to_date || 0) - new Date(b.to_date || 0)
                  )
                  .map((chair) => (
                    <TableRow key={chair.id}>
                      ...
                      <TableCell>{formatDateDisplay(chair.from_date)}</TableCell>
                      <TableCell>{formatDateDisplay(chair.to_date)}</TableCell>
                      <TableCell>
